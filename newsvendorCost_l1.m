function [cost, grad] = newsvendorCost_l1(theta, net, data, lambda, gamma, beta)
    ...
    if i<L-1
        ...
        m = length(net.layers{i}.weights);
            for j=1:m
                ...
                net.layers{i}.weights{j} = reshape(theta(count_start:count_end), w_size);
                if j < m
                    cost = cost + gamma * sum(sum(abs(net.layers{i}.weights{j})));
                end
            end
        end 
    end
        ...
        if isfield(net.layers{i}, 'weights')
            m = length(net.layers{i}.weights);
                for j=1:m
                    ...
                    grad(count_start:count_end) = dzdw(:)/N;
                    if j < m
                        grad(count_start:count_end) = grad(count_start:count_end) + gamma * sign(reshape(net.layers{i}.weights{j},[],1));
                    end
                end
        end
    end 
