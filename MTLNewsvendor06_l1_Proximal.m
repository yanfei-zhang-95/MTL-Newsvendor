...
%%
[TrainErr ,grad] = newsvendorCost_l1(theta, net, XtrainB, lambda, gamma, beta);

%% Junbin's comments   8 September 2017
% On your code, always check this test first. If this error is not at a
% scale of 10^-9 then something wrong with BP algorithm.
% func = @(alpha) newsvendorCost_l1(alpha, net, XtrainB, lambda, gamma, beta ) ; 
...
%%
maxIter = 1000;
alpha = 0.05; % learning rate for other parameters
...
for i = 1:maxIter
    ...
    [~, grad] = newsvendorCost_l1(theta, net, XtrainB, lambda, gamma, beta);
    count_end = 0;
    grad_ = cell(1,6);
    w_ = cell(1,6);
    for p = 1:L
    ...
    for k = 1:5
        [s1, s2] = size(w_{5});
        wp = w_{5} - 1/Lip*grad_{5};
        for r = 1:s1
            for s = 1:s2
                if wp(r,s) <= lambda/Lip
                    w_{5}(r,s) = 0;
                else
                    w_{5}(r,s) = wp(r,s) - gamma/Lip*sign(wp(r,s));
                end
            end
        end
    end
    ...
    % Second Update
    theta = [w_{1}(:); w_{2}(:); w_{3}(:); w_{4}(:); w_{5}(:); w_{6}(:)];
    [fval_2, grad] = newsvendorCost_l1(theta, net, XtrainB, lambda, gamma, beta);
    count_end = 0;
    grad_ = cell(1,6);
    w_ = cell(1,6);
    for p = 1:L
