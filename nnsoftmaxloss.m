function Y = nnsoftmaxloss(X,c,dzdy)
%NNSOFTMAXLOSS CNN combined softmax and logistic loss.
%   **Deprecated: use `nnloss` instead**
%
%   Y = NNSOFTMAX(X, C) applies the softmax operator followed by
%   the logistic loss the data X. X has dimension D x N,
%   packing N arrays of D-dimensional vectors.
%
%   C contains the class labels, which should be integers in the range
%   1 to D. 
%
%   DZDX = NNSOFTMAXLOSS(X, C, DZDY) computes the derivative of the
