...
O = length(unique(Ltrain_copy));

ch = rand(1, O) + 1;
cp = 3 * ch;

for k = unique(Ltrain_copy)
    Ltrain = Ltrain_copy(find(Ltrain_copy == k));
    Dtrain = Dtrain_copy(find(Ltrain_copy == k));
    XtrainB = XtrainB_copy(find(Ltrain_copy == k));
    Ltest = Ltest_copy(find(Ltest_copy == k));
    Dtest = Dtest_copy(find(Ltest_copy == k));
    XtestB = XtestB_copy(find(Ltest_copy == k));
...
net.layers{end+1} = struct('type', 'newsvendorloss', ...
                            'ch', ch(k), 'cp', cp(k), 'demands', demands) ;
...
predicted_demand_ = [predicted_demand_ predicted_demand];
disp(k);
end
...
TestErr = (n_Dtest - predicted_demand_).^2;
