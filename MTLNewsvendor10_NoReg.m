...
%%
[TrainErr,grad] = newsvendorCost_NoReg(theta, net, XtrainB, lambda, beta);

%% Junbin's comments   8 September 2017
% On your code, always check this test first. If this error is not at a
% scale of 10^-9 then something wrong with BP algorithm.

func = @(alpha) newsvendorCost_NoReg(alpha, net, XtrainB, lambda, beta ) ; 
...
[opttheta, cost, exitflag, info] = minFunc( @(p) newsvendorCost_NoReg(p, net, XtrainB, lambda, beta), theta, options);
...
[optTrainErr,~] = newsvendorCost_NoReg(opttheta, net, XtrainB, lambda, beta);
%% Prediction
[~,test_]=size(Dtest);
