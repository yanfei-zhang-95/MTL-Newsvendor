function y = nnnewsvendorloss_MTL(x, demand, cp, ch, index, dzdy, varargin)
...
if opts.leak == 0
  if nargin <= 5 || isempty(dzdy)
    ...
    temp = index .* (repmat(cp, 1, N) .* max(repmat(demand, d, 1) - x, 0.0) + repmat(ch, 1, N) .* max ( x - repmat(demand, d, 1), 0.0));  % calculate on each component  
    y = sum(temp(:));
  else
    y = dzdy.*((-repmat(cp,1,N).* ((repmat(demand,d,1)-x)>0.0)+repmat(ch,1,N).*((repmat(demand,d,1)-x)<0.0)).*index) ;
  end
